<div id="last-announcements" class="h-full">
	@if (empty($announcements))
		<div class="text-slate-400 text-sm">Sem anúncios recentes.</div>
	@else
		<div
			class="h-full overflow-y-auto max-h-44 md:max-h-none pr-2 space-y-2
			...
			@foreach ($announcements as $a)
				...
				<details class="group rounded-xl border border-slate-800 bg-slate-900/40 [&>summary::-webkit-details-marker]:hidden">
					<summary class="cursor-pointer select-none px-3 py-2 flex items-start justify-between gap-3">
						...
						<span class="shrink-0 mt-1 text-slate-300" aria-hidden="true">
							<span class="relative inline-flex items-center justify-center w-6 h-6 border border-slate-600 rounded-md">
								<span
									class="absolute h-0.5 w-3 bg-current left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"></span>
								<span
									class="absolute w-0.5 h-3 bg-current left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-transform group-open:scale-y-0 group-open:opacity-0"></span>
							</span>
