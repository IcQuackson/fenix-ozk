...
<div id="last-announcements" class="flex flex-col flex-1 min-h-0">
    ...
    @else
        <div
            class="flex-1 min-h-0 pr-2 overflow-y-auto flex flex-col gap-2
            scrollbar-thin scrollbar-track-transparent
            scrollbar-thumb-slate-400/70 hover:scrollbar-thumb-slate-500
            scrollbar-thumb-rounded-full max-h-56">
            @foreach ($announcements as $a)
                ...
                <details
                    class="group rounded-xl border border-slate-800 bg-slate-900/40
                        ...
                        <span class="shrink-0 mt-1 text-slate-300" aria-hidden="true">
                            <span
                               ...
                               border border-slate-600 rounded-md">
                                <span
                                    class="absolute h-0.5 w-3 bg-current left-1/2 top-1/2
                                 -translate-x-1/2 -translate-y-1/2"></span>
                                <span
                                    class="absolute w-0.5 h-3 bg-current left-1/2 top-1/2
